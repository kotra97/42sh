...
** Last update Wed Jun  8 09:23:49 2016 mathis rousselot
*/
...
int     get_fd_of(t_env *env, int curs)
{
  if ((env->fd.fd_in_const = open(env->arg[ARG].arg[curs + 1], O_RDONLY)) == -1)
    {
      if (errno == EACCES)
...
int	open_file(t_env *env, int curs)
{
  ...
  if (check_fd_in(env, curs) == -1)
    return (-1);
  if (get_fd_of(env, curs) == -1)
    return (-1);
  env->command[COM].in = READ_FILE;
  return (0);
}
