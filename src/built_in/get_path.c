...
** Last update Thu Jun  2 13:59:36 2016 mathis rousselot
*/
...
static void    create_path(char *prog, char *path, int *curs, char *dir)
{
  int   curs_2;
  int   curs_3;
...
static int	different_prog(char *dir, char *prog)
{
  int	curs;
  char	*str;
...
static int     different_path(char *dir, char *prog, int *curs)
{
  int	curs_2;
...
static char    *search_prog(char *prog, char *path, int exit, int curs)
{
  char  *dir;
  int   fd;
...
char	*get_path(t_env *env, char *file)
{
  ...
  if (curs != -1)
    {
      ...
      if ((dir = malloc(my_strlen(file) + curs)) == NULL)
	fatal_error_exit("Malloc failed\n");
      if (different_path(dir, file, &curs) == 0)
	return (dir);
      free(dir);
  ...
  if (env->env[env->path][5] == '\0')
    return (NULL);
  return (search_prog(file, env->env[env->path], 0, curs));
}
